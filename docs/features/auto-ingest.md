...
- **Resource limits:** LLM extraction during auto-ingest can be expensive. **Proposed:** the daemon honors a daily token budget and rate-limits extractor calls; falls back to deferring chunks rather than dropping them when the budget is exhausted.
